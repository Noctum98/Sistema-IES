<h4 class="text-info mt-3">Inscripciones</h4>
            ...
            <div class="row col-md-12">
                @if(Session::has('admin') || Session::has('regente') || Session::has('coordinador') || Session::has('seccionAlumnos'))
                    @if(!$inscripcion->aprobado)
                        <a href="{{ route('crear_usuario_alumno',['id'=>$inscripcion->id]) }}"
                           class="col-md-2 mr-2 btn btn-sm btn-success">
                            Confirmar alumno
                        </a>
                    @endif
                    <a href="{{ route('proceso.admin',['alumno_id' => $alumno->id,'carrera_id' =>$inscripcion->carrera_id,'ciclo_lectivo'=>$ciclo_lectivo]) }}"
                       class="btn btn-sm btn-light col-md-2 mr-2">Ver materias</a>

                    <button class="btn btn-sm btn-primary col-md-2 mr-2" data-bs-toggle="modal"
                            data-bs-target="#carrerasAñoModal{{$inscripcion->carrera_id}}"> Editar datos
                    </button>
                ...
                @elseif(Session::has('areaSocial'))
                    @if(Auth::user()->hasCarrera($inscripcion->carrera_id) || Session::has('areaSocial'))
                        <button class="btn btn-sm btn-warning col-md-3 mr-2" data-bs-toggle="modal"
                                data-bs-target="#carrerasMatriculacionModal{{$inscripcion->carrera_id}}">Ver
                            materias
                        </button>
                ...
                @if(Session::has('admin'))
                    <button class="btn btn-sm btn-secondary col-md-2 ml-2" data-bs-toggle="modal"
                            data-bs-target="#eliminarMatriculacionModal{{$inscripcion->carrera_id}}">
                        Eliminar
                        Inscripción
                    </button>
                ...
                @elseif(Session::has('regente') || Session::has('coordinador'))
                    @if(Auth::user()->hasCarrera($inscripcion->carrera_id))
                        <button class="btn btn-sm btn-secondary col-md-2" data-bs-toggle="modal"
                                data-bs-target="#eliminarMatriculacionModal{{$inscripcion->carrera_id}}">
