...
@section('content')
    <div class="container">
        ...
        <h4 class="text-primary">
            Inscripciones en {{$materia->nombre}} <br/>
            <span class="text-secondary ml-2">{{$instancia->nombre}}</span>
        ...
        <div class="row">
            @if(Session::has('coordinador') || Session::has('admin'))
                <button class="btn btn-sm btn-primary col-md-2 m-1" data-bs-toggle="modal"
                        data-bs-target="#inscribirAlumno"><i class="fas fa-clipboard"></i> Inscribir alumno
                </button>
        ...
        @if($mesa && !$mesa->comision_id)
            <div class="row">
                @if($mesa->cierre_profesor && !$cierre_primer_llamado)
                    <form action="{{route('mesa.abrir_acta',['mesa_id'=>$mesa->id])}}" method="POST"
                          class="col-md-1 mt-1">
                        {{method_field('PUT')}}
                ...
                @if(Session::has('admin'))
                    <button type="button" class="btn btn-sm btn-danger col-md-2 m-1" data-bs-toggle="modal"
                            data-bs-target="#borrar_mesa">Borrar Mesa
                    </button>
                    @include('mesa.modals.borrar_mesa')
                @endif


                <button class="btn btn-sm btn-secondary button-modal col-md-2 m-1" id="1" data-bs-toggle="modal"
                        data-bs-target="#libro_folio_1">
                    Libro/Folio
                </button>
                ...
                @while($contador_boton <= $mesa->folios() )
                    <a class="btn btn-sm btn-success m-1 col-md-2"
                       href="{{ route('generar_pdf_acta_volante', ['instancia' => $mesa->instancia_id, 'carrera'=>$mesa->materia->carrera_id,'materia' => $mesa->materia_id ,'llamado' => 1, 'comision' => $mesa->comision_id ?? null,'orden'=>$contador_boton]) }}">
                ...
                <table class="table mt-4">
                    ...
                    @foreach ($inscripciones as $inscripcion)
                        <tr style="cursor:pointer;">
                            ...
                            @else
                                <td>
                                    <a href="{{ route('alumno.detalle',$inscripcion->alumno_id) }}">{{ $inscripcion->nombres }}</a>
                                </td>
                            @endif
                            ...
                            <td>
                                ...
                                <button
                                    class="{{$inscripcion->confirmado ? 'd-none' : '' }} inscripcion_id btn btn-sm btn-info"
                                ...
                                @if($inscripcion->confirmado)
                                    <button class="btn btn-sm btn-success" disabled><i class="fas fa-check"></i>Confirmado
                                    </button>
                                @endif
                                @if(count($materia->mesas_instancias($instancia->id)) >= 1 && $comisiones)
                                    <button class="btn btn-sm btn-warning" data-bs-toggle="modal"
                                            data-bs-target="#asignarMesa{{$inscripcion->id}}"
                                            id="asignar-{{$inscripcion->id}}" {{ $inscripcion->mesa_id || !$inscripcion->confirmado ? 'disabled':'' }}>
                                        Asingar Mesa
                                    </button>
            ...
            @if($materia->getTotalAttribute() >= 1)
                @foreach($materia->mesas_instancias($instancia->id) as $mesa)
                    ...
                    @if(count($mesa->mesa_inscriptos()->get()) > 0)
                        ...
                        @endif
                        @if(Session::has('admin'))
                            <a class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#borrar_mesa">Borrar
                                Mesa</a>
                            @include('mesa.modals.borrar_mesa')
                        @endif
