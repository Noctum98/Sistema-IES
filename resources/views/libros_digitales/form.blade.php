...
<div class="mb-3 row">
    <label for="romanos" class="col-form-label text-lg-end col-lg-2 col-xl-3">Romanos</label>
    <div class="col-lg-10 col-xl-9">
        <input class="form-control{{ $errors->has('romanos') ? ' is-invalid' : '' }}" name="romanos" type="text"
               id="romanos" value="{{ old('romanos', optional($libroDigital)->romanos) }}" minlength="1" maxlength="191"
               required="required" placeholder="Ingrese el nro. romano aquí...">
...
<div class="mb-3 row">
    ...
    <div class="col-lg-10 col-xl-9">
        <select class="form-select{{ $errors->has('sede_id') ? ' is-invalid' : '' }}" id="sede_id" name="sede_id"
                required="required">
            <option value="" style="display: none;"
                    {{ old('sede_id', optional($libroDigital)->sede_id ?: '') == '' ? 'selected' : '' }} disabled
                    selected> Selecciones sede
            </option>
            @foreach ($Sedes as $key => $sede)
                <option
                    value="{{ $key }}" {{ old('sede_id', optional($libroDigital)->sede_id) == $key ? 'selected' : '' }}>
                    {{ $sede }}
                </option>
            @endforeach
        </select>
