    ...
    <h2 class="h1 text-info">Cargos
        <form class="me-2 d-inline-flex" action="{{ route('cargo.admin') }}">
            <div class="input-group mt-3">
                <input class="form-control mx-auto" type="text" id="search" name="search" placeholder="Buscar cargo" aria-label="Search">
                ...
                <tr>
                    ...
                    <td class="text-center">
                        ...
                        <form method="POST" class="d-inline" action="{{route('cargo.delete', ['cargo' => $cargo->id])}}">
                            @csrf
                            @method('DELETE')
                            <button class="btn btn-sm btn-danger" onclick="return confirm('Se borrará el registro. ¿Confirma?.')" type="submit"> Borrar</button>
                        </form>
                    </td>
                </tr>
                @endforeach
