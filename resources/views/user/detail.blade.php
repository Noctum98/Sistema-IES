        ...
        <div class="row col-md-12">
                    ...
                    @if(count($user->carreras) > 0)
                        @foreach($user->carreras as $carrera)
                            <li>
                                ...
                                -
                                <form action="{{ route('delete_user_carrera',$user->id) }}" method="POST"
                                      class="d-inline">
                                    {{ method_field('DELETE') }}
            ...
            <div class="col-md-12 mt-3">
                @if($sedes)
                    ...
                    @if($cargos)
                        <button type="button" class="ml-2 btn btn-sm btn-info" data-bs-toggle="modal"
                                data-bs-target="#cargosModal{{$user->id}}">
                            Asignar Cargos (Modular)
                        </button>
                        @include('user.modals.admin_carreras_cargos')
                    @endif
                @endif
                <button type="button" class="ml-2 btn btn-sm btn-primary" data-bs-toggle="modal"
                        data-bs-target="#rolesModal{{$user->id}}">
                    Asignar Roles
                </button>
                @include('user.modals.admin_roles')

                <button type="button" class="ml-2 btn btn-sm btn-primary" data-bs-toggle="modal"
                        data-bs-target="#carrerasModal{{$user->id}}">
                    Asignar Carreras
                </button>
