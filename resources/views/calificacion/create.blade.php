            ...
            <table class="table col-md-12 m-0 p-0">
                ...
                @foreach($procesos as $proceso)
                    <tr>
                            ...
                            <form action="" class="col-md-12 m-0 p-0 calificacion-alumnos form-calificacion input-group"
                                ...
                                @if($calificacion->tipo->descripcion == 1)
                                    <input type="text" style="width: 100%" class="form-control col-md-12"
                                           ...
                                           placeholder="%"
                                           @if(!Session::has('profesor') or $proceso->cierre == 1 ) disabled
                                           @endif required>
                                @else
                                    <input type="number" style="width: 100%" class="form-control col-md-12 "
                                           id="calificacion-procentaje-{{ $proceso->id }}"
                                           value="{{ $proceso->procesoCalificacion($calificacion->id) ? $proceso->procesoCalificacion($calificacion->id)->porcentaje : '' }}"
                                           placeholder="%"
                                           @if(!Session::has('profesor') or $proceso->cierre == 1 ) disabled
                        ...
                        @if($calificacion->tipo->descripcion == 1)
                            <td>
                                <form action="" class="col-md-12 m-0 p-0 calificacion-alumnos form-recuperatorio"
                                      id="{{ $proceso->id }}" method="POST">
                                    <input type="hidden" name="calificacion_id" id="calificacion_id"
                                           value="{{ $calificacion->id }}">
                                    <input type="text" style="width: 100%" class="form-control col-md-12"
