        ...
        <div class="col-4">
            <div class="dropdown">
                ...
                <ul class="dropdown-menu">
                    ...
                    <li>
                        @if (isset($cargo))
                        <a class="dropdown-item @if ($i == $ciclo_lectivo) active @endif " href="{{ route('calificacion.admin', ['materia_id' => $materia->id, 'ciclo_lectivo' => $i, 'cargo_id' => $cargo->id]) }}">{{ $i }}</a>
                        @else
                        <a class="dropdown-item @if ($i == $ciclo_lectivo) active @endif " href="{{ route('calificacion.admin', ['materia_id' => $materia->id, 'ciclo_lectivo' => $i]) }}">{{ $i }}</a>
                        @endif
                    </li>
                    @endfor
                </ul>
    ...
    @if (Session::has('profesor') || Session::has('coordinador') || Session::has('admin') || Session::has('regente') || Session::has('seccionAlumnos') || Session::has('areaSocial'))
        ...
        @foreach ($materia->comisionesCiclo($ciclo_lectivo) as $comision)
            @if (Auth::user()->hasComision($comision->id))
                @if ($cargo)
                    <a href="{{ route('proceso.listadoCargo', ['materia_id' => $materia->id, 'cargo_id' => $cargo->id, 'ciclo_lectivo' => $ciclo_lectivo, 'comision_id' => $comision->id]) }}" class="btn btn-info">
                        Ver Planilla de Calificaciones / {{ $cargo->nombre }} / {{ $comision->nombre }}
                    </a>
                @else
                    <a href="{{ route('proceso.listado', ['materia_id' => $materia->id, 'ciclo_lectivo' => $ciclo_lectivo, 'comision_id' => $comision->id]) }}" class="btn btn-info">Ver Planilla de Calificaciones {{ $comision->nombre }}</a>
                @endif
